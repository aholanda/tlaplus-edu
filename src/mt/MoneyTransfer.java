	...
	public void run() {
		...
		try {
			...
			if (type.equals( "Add")) {
				...
				if (ret == -1) {
					System.out.println("ERROR: \"Subtract operation failed, reason: not enough funds.\"");
					System.exit(-1);
				}
			} else {
				System.out.println("ERROR: Unrecognized operation: " + type);
				System.exit(-1);
			}
...
public class MoneyTransfer {
	...
	public void Next() {
		Operation A = new Operation("Subtract", accA, money);
		Operation B = new Operation("Add", accB, money);
		A.start();
		B.start();
	}
